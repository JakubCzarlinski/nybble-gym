...
@jit(nopython=True)
def reward_function(forward_factor: float,
                    angle_factor: float,
                    orientation_factor: float,
                    front_legs_factor: float,
                    horizontal_factor: float,
                    vertical_factor: float) -> np.float32:
    """Calculate reward based on the current state of the robot."""
    weights = np.array([500, -0.0025, -0.05, -0, -100, -100], dtype=np.float32)
    factors = np.array([
        forward_factor,
        angle_factor,
        orientation_factor,
...
class OpenCatGymEnv(Env):
    ...
    def __init__(self, render=False):
        ...
        self.action = np.zeros(11, dtype=np.float32)
        ...
        self.observation = np.zeros(226, dtype=np.float32)
        ...
        self.state_robot: npt.NDArray[np.float32] = np.zeros(6, dtype=np.float32)
        self.state_robot_history = np.array([], dtype=np.float32)
    ...
    def step(self,
        ...
        if self.step_counter > MAX_EPISODE_LEN or self.is_fallen():
            reward += np.float32((self.step_counter - MAX_EPISODE_LEN) / MAX_EPISODE_LEN)
            done = True
    ...
    def reset(self):
        ...
        friction = 0.5 + np.random.rand() * 0.5
        ...
        state_robot_vel = np.asarray(p.getBaseVelocity(self.robot_uid)[1], dtype=np.float32)
        state_robot_vel = state_robot_vel[0:2]
